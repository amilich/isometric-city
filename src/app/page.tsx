      ...
      >
        ...
        <T>
          <div className="flex items-center gap-3 mt-1 text-xs text-white/50">
            <span>Pop: <Var>{city.population.toLocaleString()}</Var></span>
            <span>$<Var>{city.money.toLocaleString()}</Var></span>
            <Branch branch={city.roomCode ? 'show' : 'hide'} show={<span className="text-blue-400/60"><Var>{city.roomCode}</Var></span>}><></></Branch>
          </div>
        </T>
      </button>
      {onDelete && (
