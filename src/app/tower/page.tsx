...
import { T, Branch, Num, useGT } from 'gt-next';
                  ...
                  <div key={run.id} className="relative group">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <T><span className="text-xs px-1.5 py-0.5 bg-indigo-500/20 text-indigo-200 rounded shrink-0">W<Num>{run.wave}</Num></span></T>
                      </div>
                      <div className="flex items-center gap-3 mt-1 text-xs text-white/50">
                        <T><span>$<Num>{run.money}</Num></span></T>
                        <T><span>♥ <Num>{run.lives}</Num></span></T>
                        <T><span><Num>{run.gridSize}</Num>×<Num>{run.gridSize}</Num></span></T>
                      </div>
                    </button>
                    <button
                      onClick={(e) => {
