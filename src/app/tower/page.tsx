                  ...
                  <div key={run.id} className="relative group">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs px-1.5 py-0.5 bg-indigo-500/20 text-indigo-200 rounded shrink-0">{gt('W{wave}', { wave: run.wave })}</span>
                      </div>
                      <div className="flex items-center gap-3 mt-1 text-xs text-white/50">
                        <span>{gt('${money}', { money: run.money.toLocaleString() })}</span>
                        <span>{gt('♥ {lives}', { lives: run.lives })}</span>
                        <span>{gt('{size}×{size}', { size: run.gridSize })}</span>
                      </div>
                    </button>
                    <button
                      onClick={(e) => {
