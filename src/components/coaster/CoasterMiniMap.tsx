  ...
  useEffect(() => {
    ...
    for (let y = 0; y < gridSize; y++) {
      for (let x = 0; x < gridSize; x++) {
        const tile = grid[y][x];
        let color = '#2d4a3d'; // Unowned
        
        if (tile.owned) {
          if (tile.terrain === 'water') {
            color = '#0ea5e9';
          } else if (tile.path) {
            color = tile.path.type === 'queue' ? '#9ca3af' : '#6b7280';
          } else if (tile.building) {
            if (tile.building.type === 'park_entrance') {
              color = '#a855f7';
            } else if (tile.building.type.includes('tree')) {
              color = '#22c55e';
            } else if (tile.building.type.includes('stall') || tile.building.type.includes('shop')) {
              color = '#f59e0b';
            } else {
              color = '#ec4899'; // Rides
            }
          } else {
            color = '#4ade80'; // Owned grass
          }
        }
  ...
  return (
    <Card className="fixed bottom-6 right-8 p-3 shadow-lg bg-slate-900/90 border-white/10 z-50">
      ...
      <canvas
        ...
        width={140}
        height={140}
        className="block rounded-md border border-white/20 cursor-pointer select-none"
