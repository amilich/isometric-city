...
export default function CoasterSidebar() {
  ...
  const toolGroups = useMemo<ToolGroup[]>(() => [
    ...
    { label: gt('Scenery'), tools: ['scenery_tree', 'scenery_flower', 'scenery_bench', 'scenery_shrub', 'scenery_lamp', 'scenery_statue', 'scenery_fountain', 'scenery_trash_can', 'scenery_fence'] },
    { label: gt('Rides'), tools: ['ride_carousel', 'ride_ferris_wheel', 'ride_bumper_cars', 'ride_swing', 'ride_haunted_house', 'ride_spiral_slide', 'ride_coaster_wooden', 'ride_coaster_steel'] },
    { label: gt('Shops'), tools: ['shop_food', 'shop_drink', 'shop_ice_cream', 'shop_souvenir', 'shop_info', 'shop_atm', 'shop_first_aid', 'shop_toilet'] },
  ], [gt]);
          ...
          <div key={group.label} className="mb-4">
            ...
            <div className="px-2 flex flex-col gap-0.5">
              {group.tools.map((tool) => {
                ...
                const costLabel = !isUnlocked ? gt('Research') : info.cost > 0 ? `$${info.cost}` : '';
                return (
                  <Button
                    key={tool}
