...
export function TopBar() {
  ...
  // Format month name
  const monthNames = [
    gt('Jan', { $context: 'January abbreviation' }),
    gt('Feb', { $context: 'February abbreviation' }),
    gt('Mar', { $context: 'March abbreviation' }),
    gt('Apr', { $context: 'April abbreviation' }),
    gt('May', { $context: 'May abbreviation' }),
    gt('Jun', { $context: 'June abbreviation' }),
    gt('Jul', { $context: 'July abbreviation' }),
    gt('Aug', { $context: 'August abbreviation' }),
    gt('Sep', { $context: 'September abbreviation' }),
    gt('Oct', { $context: 'October abbreviation' }),
    gt('Nov', { $context: 'November abbreviation' }),
    gt('Dec', { $context: 'December abbreviation' }),
  ];
    ...
    <div className="h-14 bg-slate-900/95 border-b border-slate-700 flex items-center px-4 gap-6">
      ...
      <div className="flex flex-col">
        ...
        <span className="text-white/50 text-xs">
          <T>
            <Var>{monthName}</Var> <Var>{day}</Var>, Year <Var>{year}</Var> — <Var>{timeString}</Var>
          </T>
        </span>
      </div>
      ...
      <div className="flex items-center gap-6 text-sm">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-white/40 text-xs"><T>Cash</T></span>
        </div>

        {/* Guests */}
        <div className="flex flex-col items-center">
          <span className="text-blue-400 font-medium">{stats.guestsInPark}</span>
          <span className="text-white/40 text-xs"><T>Guests</T></span>
        </div>

        {/* Park Rating */}
        <div className="flex flex-col items-center">
          <span className="text-yellow-400 font-medium">{stats.parkRating}</span>
          <span className="text-white/40 text-xs"><T>Rating</T></span>
        </div>
      </div>
