...
export default function GuestPanel({ guests, onClose }: GuestPanelProps) {
  ...
  const getNeedHint = (guest: Guest) => {
    ...
    if (guest.needs.bathroom < 60) return gt('Bathroom', { $context: 'Guest need indicator - needs to use bathroom' });
  ...
  const getItemLabel = (item: GuestItem) => {
    switch (item) {
      case 'food':
        return gt('Snack');
      case 'drink':
        return gt('Drink', { $context: 'Beverage item held by guest' });
      case 'souvenir':
        return gt('Souvenir');
      case 'map':
        return gt('Park Map');
  ...
  return (
    <div className="absolute top-20 right-6 z-50 w-80">
      <Card className="bg-card/95 border-border/70 shadow-xl">
        ...
        <div className="px-4 pt-4 text-xs uppercase tracking-[0.18em] text-muted-foreground">
          <T>Filters</T>
        </div>
                  ...
                  <div key={guest.id} className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground">
                        ...
                        <span>
                          <T>
                            Energy <Var>{Math.round((guest.needs.energy / 255) * 100)}</Var>%
                          </T>
                        </span>
                        {needHint && (
