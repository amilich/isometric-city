...
import { T, Var, useGT, useMessages, msg } from 'gt-next';
...
const STATE_LABELS: Record<string, string> = {
  wandering: msg('wandering'),
  queuing: msg('queuing'),
  heading_to_ride: msg('heading to ride'),
  on_ride: msg('on ride'),
  at_shop: msg('at shop'),
  leaving_park: msg('leaving park'),
};
                ...
                return (
                  <div key={guest.id} className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground capitalize">{m(STATE_LABELS[guest.state] ?? guest.state.replace('_', ' '))}</div>
