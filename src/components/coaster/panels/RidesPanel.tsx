                    ...
                    <div>
                      ...
                      <T>
                        <div className="text-xs text-muted-foreground">
                          Queue <Var>{ride.queue.guestIds.length}</Var> / <Var>{ride.queue.maxLength}</Var> ·{' '}
                      ...
                      <div className="text-xs text-muted-foreground">
                        {estimatedWait > 0 ? gt('{estimatedWait} min wait', { estimatedWait }) : gt('No wait')} · {gt('${price} ticket', { price: ride.price })} ·{' '}
                        {gt('{reliability}% reliable', { reliability: Math.round(ride.stats.reliability * 100) })}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {gt('{uptime}% uptime', { uptime: Math.round(ride.stats.uptime * 100) })} · {gt('{totalRiders} riders', { totalRiders: ride.stats.totalRiders })}
                      </div>
                    </div>
