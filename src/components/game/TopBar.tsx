    ...
    <div className="h-14 bg-card border-b border-border flex items-center justify-between px-4">
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-0 bg-secondary rounded-md p-0">
          {[0, 1, 2, 3].map(s => {
            const label = s === 0 ? 'Pause' : s === 1 ? 'Normal' : s === 2 ? 'Fast' : 'Very Fast';
            const shortcut = s === 0 ? KEYBOARD_SHORTCUTS.PAUSE.label : undefined;
            
            return (
              <ShortcutTooltip key={s} content={label} shortcut={shortcut}>
                <Button
