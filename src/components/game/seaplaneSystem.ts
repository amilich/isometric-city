    ...
    for (const seaplane0 of seaplanesRef.current) {
      ...
      if (seaplane.altitude > 0.7) {
        ...
        if (seaplane.stateProgress >= contrailSpawnInterval) {
          ...
          seaplane.contrail = [...seaplane.contrail, { x: contrailX, y: contrailY, age: 0, opacity: 1 }];
        }
      }
