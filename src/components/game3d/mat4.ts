...
export const mat4Ortho = (out: Mat4, left: number, right: number, bottom: number, top: number, near: number, far: number): Mat4 => {
  out.fill(0);
  out[0] = 2 / (right - left);
  out[5] = 2 / (top - bottom);
  out[10] = -2 / (far - near);
...
export const mat4LookAt = (out: Mat4, eye: Vec3, center: Vec3, up: Vec3): Mat4 => {
...
export const mat4Invert = (out: Mat4, m: Mat4): Mat4 | null => {
...
export const transformPoint = (out: Vec3, m: Mat4, x: number, y: number, z: number): Vec3 => {
