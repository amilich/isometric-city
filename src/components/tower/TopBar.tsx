      ...
      <div className="flex items-center gap-3">
        <T>
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-mono text-amber-400">$<Var>{money.toLocaleString()}</Var></span>
          </div>
        </T>
        <T>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-muted-foreground">Lives</span>
            <span className={`font-mono ${lives > 0 ? 'text-green-400' : 'text-red-400'}`}><Var>{lives}</Var></span>
          </div>
        </T>
        <T>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-muted-foreground">Wave</span>
            <span className="font-mono text-white/90"><Var>{stats.wave}</Var></span>
            <span className="text-xs text-muted-foreground">(<Var>{waveLabel}</Var>)</span>
          </div>
        </T>
      </div>
