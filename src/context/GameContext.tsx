...
export function GameProvider({ children, startFresh = false }: { children: React.ReactNode; startFresh?: boolean }) {
  ...
  const placeCustomBuilding = useCallback((x: number, y: number, customBuilding: CustomBuildingData): boolean => {
    ...
    setState((prev) => {
      ...
      // Check if all tiles in the footprint are valid and empty
      for (let dy = 0; dy < size; dy++) {
        for (let dx = 0; dx < size; dx++) {
          const tile = prev.grid[y + dy]?.[x + dx];
          if (!tile) return prev; // Out of bounds
          // Allow placement on grass or empty, not on water/roads/other buildings
          const type = tile.building.type;
          if (type !== 'grass' && type !== 'empty') return prev;
        }
      }
          ...
          if (colX >= x && colX < x + size && rowY >= y && rowY < y + size) {
            ...
            return {
              ...
              building: {
                ...
                customSize: size,
                level: 1,
                population: isOrigin ? customBuilding.stats.maxPop : 0,
                jobs: isOrigin ? customBuilding.stats.maxJobs : 0,
