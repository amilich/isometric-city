...
import { createRoNRealisticTerrainRenderer, hashStringToSeed } from '../lib/realisticTerrain';
...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  // High-fidelity terrain renderer (RoN-only)
  const realisticTerrainRef = useRef<ReturnType<typeof createRoNRealisticTerrainRenderer> | null>(null);
  const realisticTerrainSeedRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // High-fidelity renderer initialization (kept RoN-only)
      const hiFiEnabled = graphics.quality === 'high';
      if (hiFiEnabled) {
        const seed = hashStringToSeed(gameState.id);
        if (!realisticTerrainRef.current || realisticTerrainSeedRef.current !== seed) {
          realisticTerrainRef.current = createRoNRealisticTerrainRenderer({ seed });
          realisticTerrainSeedRef.current = seed;
        }
      }
      const terrainRenderer = hiFiEnabled ? realisticTerrainRef.current : null;
      const waterDepthMap = terrainRenderer ? terrainRenderer.getWaterDepthMap(gameState.grid, gameState.gridSize) : null;

      // Draw sky background (optionally dynamic)
      const skyTime = (() => {
        if (graphics.timeOfDay !== 'dynamic') {
          // Map RoN setting to shared renderer times
          return graphics.timeOfDay === 'night' ? 'night' : graphics.timeOfDay === 'dusk' ? 'dusk' : 'day';
        }
        // Dynamic cycle (subtle): day -> dusk -> night
        const phase = (nowSeconds / 90) % 1;
        if (phase < 0.62) return 'day';
        if (phase < 0.78) return 'dusk';
        return 'night';
      })();
      drawSkyBackground(ctx, canvas, skyTime);
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            ...
            if (terrainRenderer && waterDepthMap) {
              // Land adjacency for shoreline foam (exclude docks)
              const adjacentLand = {
              ...
              const depth = waterDepthMap[y * gameState.gridSize + x] ?? 0;
              terrainRenderer.drawWaterTile(ctx, screenX, screenY, x, y, depth, adjacentWater, adjacentLand, nowSeconds, graphics.timeOfDay);
            } else {
            ...
            if (terrainRenderer) {
              const playerIndex = tile.ownerId ? gameState.players.findIndex(p => p.id === tile.ownerId) : -1;
              const ownerTint = playerIndex >= 0 ? PLAYER_COLORS[playerIndex] : null;
              const isIndustrialAge = AGE_ORDER.indexOf(playerAge) >= AGE_ORDER.indexOf('industrial');

              terrainRenderer.drawLandTile(ctx, screenX, screenY, x, y, tile, nowSeconds, graphics.timeOfDay, {
                isRoadUnderlay: tile.building?.type === 'road',
                ownerTint,
                showOil: isIndustrialAge,
              });

              if (tile.forestDensity > 0) {
                terrainRenderer.drawForestOverlay(ctx, screenX, screenY, x, y, tile.forestDensity, nowSeconds, graphics.timeOfDay);
                // Draw trees on forest tiles using IsoCity's tree sprite (kept, but grounded by new underlay)
                ...
                if (isoCitySprite) {
                  ...
                  const wind = Math.sin(nowSeconds * 1.2 + (x + y) * 0.15) * 0.6;
                  ...
                  for (let t = 0; t < numTrees; t++) {
                    ...
                    const treeDrawX = screenX + TILE_WIDTH * pos.dx - treeDestWidth / 2 + offsetX + wind * (0.35 + t * 0.05);
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            if (terrainRenderer) {
              terrainRenderer.drawBeachOnWater(ctx, screenX, screenY, adjacentLand, nowSeconds, graphics.timeOfDay);
            } else {
