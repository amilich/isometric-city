...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const animTime = fireAnimTimeRef.current; // Use existing animation time ref
      drawEnhancedSky(ctx, canvas, { timeOfDay: 'day', time: animTime });
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            ...
            // Use enhanced water rendering with depth, waves, and sparkles
            ...
            // Use enhanced water rendering for docks too
            drawEnhancedWaterTile(ctx, screenX, screenY, x, y, adjacentWater, {
              time: animTime,
              zoom: currentZoom,
              depth: 0.4, // Shallower depth for dock areas
            });
          } else {
            ...
            if (tile.hasMetalDeposit) {
              // Use enhanced mountain rendering with procedural noise, snow caps, and ore glints
              ...
              // Only show oil deposits in industrial+ ages, otherwise just draw enhanced grass
              const isIndustrial = AGE_ORDER.indexOf(playerAge) >= AGE_ORDER.indexOf('industrial');
              if (isIndustrial) {
                // Use enhanced oil deposit rendering with iridescent sheen
                drawEnhancedOilDeposit(ctx, screenX, screenY, x, y, {
                  zoom: currentZoom,
                  time: animTime,
                });
              } else {
                // Pre-industrial: just show enhanced grass tile
                drawEnhancedGroundTile(ctx, screenX, screenY, x, y, {
                  zoom: currentZoom,
                  time: animTime,
                });
              }
            } else if (tile.forestDensity > 0) {
              // Use enhanced forest rendering with procedural trees, wind sway, and shadows
              // Get optional tree sprite for hybrid rendering
              const isoCitySprite = getCachedImage(ISOCITY_SPRITE_PATH, true);
              drawEnhancedForest(ctx, screenX, screenY, x, y, tile.forestDensity, {
                zoom: currentZoom,
                time: animTime,
                treeSprite: isoCitySprite,
              });
              ...
              // Regular enhanced grass tile with noise-based variation
              // Get player color for territory tinting
              const playerIndex = tile.ownerId ? gameState.players.findIndex(p => p.id === tile.ownerId) : -1;
              const playerColor = playerIndex >= 0 ? PLAYER_COLORS[playerIndex] : null;
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          // Draw enhanced beach with wet/dry gradients and animated foam
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            drawEnhancedBeach(ctx, screenX, screenY, adjacentLand, {
              time: animTime,
              zoom: currentZoom,
            });
          }
