...
// Import enhanced graphics for realistic rendering
import {
...
import { drawEnhancedUnit, getEnhancedPlayerColor } from '../lib/enhancedUnits';
...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  // Enhanced graphics mode (enabled by default for realistic rendering)
  const enhancedGraphicsRef = useRef(true);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw sky background (enhanced with clouds and atmosphere)
      if (enhancedGraphicsRef.current) {
        drawEnhancedSky(ctx, canvas.width, canvas.height, 'day', fireAnimTimeRef.current);
      } else {
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            // Check adjacent water/land tiles
            const adjacentWater = {
            ...
            const waterDepth = 1 - landCount * 0.25;
            
            if (enhancedGraphicsRef.current) {
              drawEnhancedWaterTile(ctx, screenX, screenY, x, y, currentZoom, fireAnimTimeRef.current, waterDepth, adjacentLand);
            } else {
            ...
            if (tile.hasMetalDeposit) {
              ...
              if (enhancedGraphicsRef.current) {
                // Use enhanced mountain rendering with realistic shading, snow, and ore glints
                drawEnhancedMountain(ctx, screenX, screenY, x, y, true, currentZoom, fireAnimTimeRef.current);
              } else {
                // Fallback to original mountain rendering
              ...
              if (enhancedGraphicsRef.current) {
                drawEnhancedGrassTile(ctx, screenX, screenY, x, y, currentZoom, fireAnimTimeRef.current);
              } else {
              ...
              // Draw trees on forest tiles
              if (enhancedGraphicsRef.current) {
                // Use enhanced procedural forest with wind animation
                drawEnhancedForest(ctx, screenX, screenY, x, y, tile.forestDensity / 100, currentZoom, fireAnimTimeRef.current);
              } else {
                // Fallback to IsoCity tree sprites
              ...
              // Regular grass tile with enhanced rendering
              if (enhancedGraphicsRef.current) {
                drawEnhancedGrassTile(ctx, screenX, screenY, x, y, currentZoom, fireAnimTimeRef.current);
              } else {
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            if (enhancedGraphicsRef.current) {
              drawEnhancedBeach(ctx, screenX, screenY, x, y, adjacentLand, fireAnimTimeRef.current);
            } else {
      ...
      // Fourth pass: Draw units with enhanced sprites
      gameState.units.forEach(unit => {
        ...
        // Use enhanced unit rendering for realistic graphics
        if (enhancedGraphicsRef.current) {
          drawEnhancedUnit(ctx, unit, 0, 0, currentZoom, playerIndex, gameState.tick);
        } else {
