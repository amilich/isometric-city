...
import { drawEnhancedWaterTile, drawEnhancedBeach, calculateWaterDepth } from '../lib/enhancedWater';
import { drawEnhancedGrassTile, drawEnhancedForest, drawEnhancedMountain, drawEnhancedOilDeposit, drawEnhancedGreyBase } from '../lib/enhancedTerrain';
import { initAmbientEffects, updateAmbientEffects, drawAmbientEffects, drawWeatherEffects } from '../lib/ambientEffects';
import { getLightingForTime, applyAmbientLighting, applyVignette, drawBuildingShadow, updateLightingEffects, drawLightingEffects, LightingState, LIGHTING_PRESETS } from '../lib/lightingSystem';
import { drawEnhancedSelectionRing, drawEnhancedHealthBar, drawDustTrail } from '../lib/enhancedUnits';
...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  const gameTimeRef = useRef(0); // Total game time for animations
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (!ambientInitializedRef.current && gameState.gridSize > 0) {
        initAmbientEffects(gameState.gridSize, gameState.gridSize);
      ...
      updateAmbientEffects(
        ...
        gameState.gridSize,
        gameState.gridSize,
        'summer', // Could be dynamic based on game state
        'day',    // Could be dynamic based on game time
        'clear',  // Could be dynamic based on game state
        canvas.width / dpr,
        canvas.height / dpr
      );
      ...
      // Get current lighting state (could be time-based)
      const lighting = LIGHTING_PRESETS.noon;
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            // Check adjacent land tiles for coastlines
            const hasCoastNorth = x > 0 && gameState.grid[y]?.[x - 1]?.terrain !== 'water';
            const hasCoastEast = y > 0 && gameState.grid[y - 1]?.[x]?.terrain !== 'water';
            const hasCoastSouth = x < gameState.gridSize - 1 && gameState.grid[y]?.[x + 1]?.terrain !== 'water';
            const hasCoastWest = y < gameState.gridSize - 1 && gameState.grid[y + 1]?.[x]?.terrain !== 'water';
            
            // Calculate water depth based on distance from coast
            const isWaterTile = (wx: number, wy: number) => 
              wx >= 0 && wx < gameState.gridSize && wy >= 0 && wy < gameState.gridSize && 
              gameState.grid[wy]?.[wx]?.terrain === 'water';
            const depth = calculateWaterDepth(x, y, gameState.gridSize, isWaterTile);
            
            // Use enhanced water rendering
            drawEnhancedWaterTile(ctx, screenX, screenY, x, y, {
            ...
            // Use enhanced water for consistent look
            const isWaterTile = (wx: number, wy: number) => 
              wx >= 0 && wx < gameState.gridSize && wy >= 0 && wy < gameState.gridSize && 
              (gameState.grid[wy]?.[wx]?.terrain === 'water' || hasDock(gameState.grid, wx, wy, gameState.gridSize));
            const depth = calculateWaterDepth(x, y, gameState.gridSize, isWaterTile);
            ...
            if (tile.hasMetalDeposit) {
              // Use enhanced mountain rendering
              drawEnhancedMountain(ctx, screenX, screenY, x, y, {
                time: gameTimeRef.current,
                tileWidth: TILE_WIDTH,
                tileHeight: TILE_HEIGHT,
                zoom: currentZoom,
              ...
              if (isIndustrial) {
                // Use enhanced oil deposit rendering (animated)
                drawEnhancedOilDeposit(ctx, screenX, screenY, x, y, {
                  time: gameTimeRef.current,
                  tileWidth: TILE_WIDTH,
                  tileHeight: TILE_HEIGHT,
                  zoom: currentZoom,
                ...
                // Before industrial age, just show grass
                drawEnhancedGrassTile(ctx, screenX, screenY, x, y, {
                  time: gameTimeRef.current,
                  tileWidth: TILE_WIDTH,
                  tileHeight: TILE_HEIGHT,
                  zoom: currentZoom,
              ...
              // Use enhanced forest rendering with varied tree types
              drawEnhancedForest(ctx, screenX, screenY, x, y, tile.forestDensity, {
                time: gameTimeRef.current,
                tileWidth: TILE_WIDTH,
                tileHeight: TILE_HEIGHT,
                zoom: currentZoom,
              ...
              // Regular grass tile - use enhanced version
              drawEnhancedGrassTile(ctx, screenX, screenY, x, y, {
                time: gameTimeRef.current,
                tileWidth: TILE_WIDTH,
                tileHeight: TILE_HEIGHT,
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            drawEnhancedBeach(ctx, screenX, screenY, gameTimeRef.current, TILE_WIDTH, TILE_HEIGHT, adjacentLand);
          }
        }
      }
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (needsGreyBase) {
            ...
            drawEnhancedGreyBase(ctx, screenX, screenY, buildingSize.width, buildingSize.height, TILE_WIDTH, TILE_HEIGHT);
          }
      ...
      gameState.units.forEach(unit => {
        ...
        // Draw dust trail for moving units
        if (unit.isMoving) {
          drawDustTrail(ctx, screenX + TILE_WIDTH / 2, screenY + TILE_HEIGHT / 2, 0.6, gameTimeRef.current);
        }
        
        // Draw unit with pedestrian-like appearance and task activities
        drawRoNUnit(ctx, unit, 0, 0, currentZoom, color, gameState.tick);
        
        // Draw enhanced selection ring for selected units
        if (unit.isSelected) {
          drawEnhancedSelectionRing(
            ctx,
            screenX + TILE_WIDTH / 2,
            screenY + TILE_HEIGHT / 2 + 5,
            12,
            color,
            gameTimeRef.current
          );
        ...
        // Draw enhanced health bar for damaged units
        if (unit.health < unit.maxHealth) {
          drawEnhancedHealthBar(
            ctx,
            screenX + TILE_WIDTH / 2,
            screenY - 5,
            24,
      ...
      // Fifth pass: Draw ambient effects (birds, particles, cloud shadows)
      const gridToScreenFunc = (gx: number, gy: number) => gridToScreen(gx, gy, 0, 0);
      drawAmbientEffects(ctx, gridToScreenFunc, viewBounds, gameTimeRef.current);
      
      // Draw lighting effects (explosions, muzzle flashes, light sources)
      drawLightingEffects(ctx, gridToScreenFunc, viewBounds, gameTimeRef.current, lighting);
      
      ctx.restore();
      
      // Screen-space effects (after world transform)
      // Apply subtle vignette for atmosphere
      applyVignette(ctx, canvas.width / dpr, canvas.height / dpr, 0.15);
      
      // Draw weather effects (rain/snow - screen space)
      drawWeatherEffects(ctx, 'clear');
