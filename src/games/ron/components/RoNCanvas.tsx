...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      drawEnhancedSkyBackground(ctx, canvas, 'day', fireAnimTimeRef.current);
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            // Check adjacent water tiles for shore detection
            const adjacentWater = {
            ...
            // Use enhanced realistic water with animated waves and depth
            drawRealisticWaterTile(
              ctx, screenX, screenY, x, y,
              fireAnimTimeRef.current,
              adjacentWater,
              { hasFishingSpot: tile.hasFishingSpot }
            );
            ...
            drawRealisticWaterTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, adjacentWater, {});
          } else {
            // Enhanced terrain rendering with realistic procedural graphics
            if (tile.hasMetalDeposit) {
              // Draw realistic mountain terrain with ore deposits
              drawMountainTile(ctx, screenX, screenY, x, y, true);
            } else if (tile.hasOilDeposit) {
              // Draw grass base first with realistic grass
              drawRealisticGrassTile(ctx, screenX, screenY, x, y);
              ...
              if (isIndustrial) {
                ...
                // Draw realistic oil splotches with sheen
                for (let i = 0; i < numSplotches; i++) {
                  const splotchSeed = seed * 7 + i * 13;
                  const baseSize = 0.08 + (splotchSeed % 60) / 1000;
                  const sW = TILE_WIDTH * baseSize;
                  const sH = TILE_HEIGHT * (baseSize * 0.8 + (splotchSeed * 2 % 30) / 1000);
                  const sDx = ((splotchSeed % 70) - 35) / 100 * TILE_WIDTH * 0.55;
                  const sDy = ((splotchSeed * 3 % 50) - 25) / 100 * TILE_HEIGHT * 0.55;
                  const sAngle = ((splotchSeed * 5) % 90 - 45) * Math.PI / 180;
                  
                  const px = cx + sDx;
                  const py = cy + sDy;
                  
                  // Dark oil with iridescent sheen
                  const darkness = 8 + (i * 2 % 6);
                  ctx.fillStyle = `rgb(${darkness}, ${darkness}, ${darkness + 4})`;
                  ctx.beginPath();
                  ctx.ellipse(px, py, sW, sH, sAngle, 0, Math.PI * 2);
                  ctx.fill();
                  
                  // Rainbow sheen effect
                  const sheenGradient = ctx.createRadialGradient(
                    px - sW * 0.2, py - sH * 0.2, 0,
                    px, py, sW
                  );
                  sheenGradient.addColorStop(0, 'rgba(160, 100, 180, 0.15)');
                  sheenGradient.addColorStop(0.5, 'rgba(100, 180, 160, 0.1)');
                  sheenGradient.addColorStop(1, 'rgba(50, 50, 70, 0.05)');
                  ctx.fillStyle = sheenGradient;
                  ctx.beginPath();
                  ctx.ellipse(px, py, sW * 0.8, sH * 0.7, sAngle, 0, Math.PI * 2);
                  ctx.fill();
                }
              }
            } else if (tile.forestDensity > 0) {
              // Draw realistic forest tile with procedural trees and wind sway
              drawForestTile(ctx, screenX, screenY, x, y, tile.forestDensity, fireAnimTimeRef.current);
            } else if (tile.building?.type === 'road') {
              // Draw realistic grass base under roads
              drawRealisticGrassTile(ctx, screenX, screenY, x, y);
            } else {
              // Regular realistic grass tile
              drawRealisticGrassTile(ctx, screenX, screenY, x, y);
            }
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          // Draw realistic beach with animated foam if any adjacent tile is land (and not a dock)
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            drawBeachTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, adjacentLand);
          }
        }
      }
