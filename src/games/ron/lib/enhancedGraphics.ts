...
export const ENHANCED_GRASS_COLORS = {
  ...
  // Tuned to be more realistic (muted olive/earth tones; less "cartoon green")
  base: { h: 98, s: 28, l: 32 },
  light: { h: 92, s: 26, l: 40 },
  dark: { h: 106, s: 22, l: 22 },
  accent: { h: 82, s: 20, l: 34 },
  // Border/stroke
  stroke: 'rgba(20, 35, 20, 0.22)',
};

/** Enhanced water color palette */
export const ENHANCED_WATER_COLORS = {
  // Tuned for a deeper, less-saturated "real water" look
  deep: { h: 212, s: 55, l: 18 },
  mid: { h: 206, s: 50, l: 30 },
  shallow: { h: 196, s: 40, l: 45 },
  foam: { h: 200, s: 15, l: 92 },
  reflection: { h: 210, s: 20, l: 82 },
  sparkle: '#ffffff',
};

/** Enhanced sand/beach color palette */
export const ENHANCED_BEACH_COLORS = {
  // Tuned to be less orange and more natural sand
  dry: { h: 38, s: 30, l: 70 },
  wet: { h: 35, s: 28, l: 50 },
  dark: { h: 30, s: 22, l: 40 },
  foam: { h: 45, s: 15, l: 90 },
...
export const ENHANCED_FOREST_COLORS = {
  canopy: { h: 122, s: 35, l: 18 },
  highlight: { h: 112, s: 32, l: 28 },
  shadow: { h: 132, s: 25, l: 12 },
  trunk: { h: 25, s: 35, l: 22 },
};
...
export function drawEnhancedGrassTile(
  ...
  // Base albedo (no obvious "green gradient tile" look)
  ctx.fillStyle = hsl(baseH, baseS, finalL);
  ctx.beginPath();
  ...
  // Lighting + micro-variation (subtle; avoids looking like a flat tile)
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(cx, screenY);
  ctx.lineTo(screenX + w, cy);
  ...
  key.addColorStop(0, 'rgba(255,255,255,0.08)');
  key.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.globalCompositeOperation = 'screen';
  ctx.fillStyle = key;
  ctx.fillRect(screenX, screenY, w, h);

  // Edge ambient occlusion (slightly darker near edges)
  const ao = ctx.createRadialGradient(cx, cy, 0, cx, cy, w * 0.9);
  ...
  // Texture (only when zoomed in enough; keep it cheap)
  if (zoom >= 0.6) {
    ctx.globalCompositeOperation = 'overlay';

    // Small speckles + faint dry patches
    const speckles = 10 + Math.floor((detailVal + 1) * 3);
    for (let i = 0; i < speckles; i++) {
      const seed = (gridX * 171 + gridY * 313 + i * 97) % 997;
      const rx = ((seed * 37) % 1000) / 1000;
      const ry = ((seed * 91) % 1000) / 1000;
      const px = screenX + rx * w;
      const py = screenY + ry * h;
      const n = detailNoise((gridX + rx) * 3.1, (gridY + ry) * 3.1);

      // Slight hue drift toward dry grass (more yellow) or lush (more green)
      const hueDrift = lerp(-8, 6, (n + 1) / 2);
      const lDrift = lerp(-10, 8, (n + 1) / 2);
      ctx.fillStyle = hsl(baseH + hueDrift, baseS * 0.9, Math.max(10, finalL + lDrift), 0.08);
      ctx.beginPath();
      ctx.arc(px, py, 0.6 + ((seed % 3) * 0.3), 0, Math.PI * 2);
      ctx.fill();
    }
  }
