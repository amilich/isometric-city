...
// Staff member name message patterns with ID placeholder
export function getStaffNameMessage(type: StaffType, id: number): string {
  const patterns: Record<StaffType, string> = {
    handyman: msg('Handyman {id}', { id: String(id) }),
    mechanic: msg('Mechanic {id}', { id: String(id) }),
    security: msg('Security {id}', { id: String(id) }),
    entertainer: msg('Entertainer {id}', { id: String(id) }),
  };
